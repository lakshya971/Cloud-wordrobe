...
export interface User {
  ...
  isVendor: boolean;
  vendorPaid: boolean;
  vendorInfo?: string; // JSON string containing vendor business details
  createdAt: Date;
  updatedAt: Date;
}
    ...
    if (await isDatabaseAvailable()) {
      ...
      if (db) {
        const [result] = await db.execute(
          'INSERT INTO users (email, password, name, phone, role, isVendor, vendorPaid, vendorInfo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [userData.email, userData.password, userData.name, userData.phone, userData.role, userData.isVendor, userData.vendorPaid, userData.vendorInfo || null]
        );
...
export async function updateUser(userId: number, updateData: Partial<User>): Promise<boolean> {
  try {
    if (await isDatabaseAvailable()) {
      const db = getDB();
      if (db) {
        ...
        const query = `UPDATE users SET ${fields.join(', ')}, updatedAt = NOW() WHERE id = ?`;
    ...
    console.error('Database updateUser failed:', error);
  }
